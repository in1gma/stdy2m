...
\begin{abstract}
 When multiple physical frequencies are available, it also achieves increased energy efficiency, demonstrating the ability to work against radio interference and the tolerance to a wide range of measured time synchronization errors.
A. Stankovic, Computer Science Department, University of Virginia; T. Yan, Eaton Innovation Center; T. He, Computer Science Department, University of Minnesota; C. Huang, Google; T. F. Abdelzaher, (Current address) NASA Ames Research Center, Moffett Field, California 94035.
...
As Algorithm <ref> states, for each frequency number, each node calculates a random number (Rnd_α) for itself and a random number (Rnd_β) for each of its two-hop neighbors with the same pseudorandom number generator.
Node α's ID (ID_α), and node α's neighbors' IDs within two communication hops.The frequency number (FreNum_α) node α gets assigned.
Why don't we just map nodes' IDs within two hops into a group of frequency numbers and assign those numbers to all nodes within two hops?
Our simulation is configured according to the settings in Table <ref>.
Node Number     & 289
Radio Range     & 20m--45m Source:This is a table sourcenote.
In this appendix, we measure the channel switching time of Micaz [CROSSBOW] sensor devices. 
\end{abstract}
...
sidered in controlling duty-cycles in the
MAC layer.
